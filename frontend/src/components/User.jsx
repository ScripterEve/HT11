  ...
  return (
    <div className="bg-[#FBFFE4] min-h-screen flex flex-col">
      ...
      <div className="px-6 mt-10 flex-grow pb-20">
        <h2 className="text-4xl font-bold mb-10 text-[#3D8D7A]">Saved Recipes</h2>
        {savedRecipes.length > 0 ? (
          <div className="grid grid-cols-[repeat(auto-fill,_minmax(290px,_1fr))] gap-6">
            {savedRecipes.map((recipe, index) => (
              <div
                key={index}
                className="bg-white shadow-lg rounded-xl p-6 hover:scale-105 transition-transform duration-300 border border-[#3D8D7A] relative"
              >
                <button
